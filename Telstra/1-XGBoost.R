...
# Replace NAs to -1 in 'Event Type' feature
...
# Extract 10% of data from Train dataset for cross-validation and to monitor the performance of model
trainIndex 	<- createDataPartition(train$location, p=0.1, list=FALSE) 
dtest 		<- train[ trainIndex, ]
train 		<- train[-trainIndex, ]

# Extract Predictor from dtest dataset
dtestpredictor 	<- dtest[, predictorname]

# Extract Predictor from train dataset
...
# Create DMatrix object for dtest dataset - Will be used for watch list. 
dtestmatrix <- xgb.DMatrix(data = data.matrix(dtest), label = dtestpredictor) 

# Create XGB Matrix for Train dataset
