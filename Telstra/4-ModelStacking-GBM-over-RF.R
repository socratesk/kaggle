...
trainIndex <- createDataPartition(train$location, p=0.6, list=FALSE) 

# Split train dataset to train1 and train2
trainRF 	<- train[ trainIndex, ] 
trainGBM 	<- train[-trainIndex, ] 
targetRF	<- trainpredictor[ trainIndex] 
targetGBM	<- trainpredictor[-trainIndex] 

# Remove unwanted objects and garbage collect
trainIndex 	<- NULL
train 		<- NULL
trainold 	<- NULL
resource_df	<- NULL
...
# Train GBM Model
gbmModel <- train(x 		=  as.data.frame(trainGBM)
		, y 		= targetGBM
		, method 	= 'gbm'
		, trControl 	= traincontrol
		, tuneGrid 	= traingrid
...
write.csv(resp, 'Submission-4-GBMOverRF.csv', row.names=F, quote = F)
